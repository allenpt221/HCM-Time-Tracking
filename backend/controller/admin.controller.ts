...
export async function updatePunch(req: Request, res: Response) {
    try {
        ...
        if (punchIn !== undefined) updateData.punchIn = Timestamp.fromDate(new Date(punchIn));
        if (punchOut !== undefined) updateData.punchOut = Timestamp.fromDate(new Date(punchOut));
...
export async function getAllAttendance(req: Request, res: Response) {
    try {
        ...
        })) as Array<{ id: string; userId: string; date: string; [key: string]: any }>;
